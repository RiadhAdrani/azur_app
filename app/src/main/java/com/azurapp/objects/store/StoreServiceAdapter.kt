...
class StoreServiceAdapter(
...
) : RecyclerView.Adapter<StoreServiceAdapter.StoreHolder>() {
    ...
    inner class StoreHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        private val service: TextView = itemView.findViewById(R.id.item_service)

        fun bind(store: Store) {
            logo.setImageResource(store.icon)
            name.text = context.getText(store.name)
            service.text = context.getText(store.activity[1].name)
    ...
    override fun onCreateViewHolder(
        ...
        return StoreHolder(
            LayoutInflater.from(parent.context)
                .inflate(R.layout.item_store_service, parent, false)
        )
    }
